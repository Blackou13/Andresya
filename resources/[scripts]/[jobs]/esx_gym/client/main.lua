...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour louer un ~b~velo')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez ~INPUT_CONTEXT~ pour ouvrir le ~b~gym~w~ menu')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour exercer vos ~g~bras')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Preparer l' ~g~exercice~w~...")
						Citizen.Wait(1000)					
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez ~INPUT_CONTEXT~ pour faire des ~g~tractions')
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Nous sommes ouvert ~g~24~w~ heures/jour. Bienvenue !")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Protéine (20$)', value = 'protein_shake'},
				{label = 'Eau (5$)', value = 'water'},
				{label = 'Déjeuner sportif (50$)', value = 'sportlunch'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Gym - Abonnement',
            elements = {
				{label = 'Abonnement (500$)', value = 'membership'},
            }
        },
        function(data, menu)
...
function OpenBikeMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Louer un velo (50$)',
            elements = {
				{label = 'BMX', value = 'bmx'},
				{label = 'Balade', value = 'cruiser'},
				{label = 'Course', value = 'fixter'},
				{label = 'Montagne', value = 'scorcher'},
            }
        },
        function(data, menu)
