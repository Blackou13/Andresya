...
Config5599.Map = {
  ...
  {name="Magasin de masque", color=83, id=362, x=-1336.341, y=-1277.929, z=29.200},
  {name="Magasin de vêtements", color=60, id=73, x=88.291, y=-1391.929, z=29.200},
  {name="Magasin de vêtements", color=60, id=73, x=-718.958, y=-158.059, z=36.996},
  {name="Magasin de vêtements", color=60, id=73, x=-151.204, y=-306.837, z=38.724},
  {name="Magasin de vêtements", color=60, id=73, x=414.646, y=-807.452, z=29.338},
  {name="Magasin de vêtements", color=60, id=73, x=-815.193, y=-1083.333, z=11.022},
  {name="Magasin de vêtements", color=60, id=73, x=-1208.098, y=-782.020, z=17.163},
  {name="Magasin de vêtements", color=60, id=73, x=-1457.954, y=-229.426, z=49.158},
  {name="Magasin de vêtements", color=60, id=73, x=-2.777, y=6518.491, z=31.533},
  {name="Magasin de vêtements", color=60, id=73, x=1681.586, y=4820.133, z=42.046},
  {name="Magasin de vêtements", color=60, id=73, x=130.216, y=-202.940, z=54.505},
  {name="Magasin de vêtements", color=60, id=73, x=618.701, y=2740.564, z=41.905},
  {name="Magasin de vêtements", color=60, id=73, x=1199.169, y=2694.895, z=37.866},
  {name="Magasin de vêtements", color=60, id=73, x=-3164.172, y=1063.927, z=20.674},
  {name="Magasin de vêtements", color=60, id=73, x=-1091.373, y=2702.356, z=19.422},

}
Citizen.CreateThread(function()

    for i = 1, #Config5599.Map, 1 do
        local blip = AddBlipForCoord(Config5599.Map[i].x, Config5599.Map[i].y, Config5599.Map[i].z)
        SetBlipSprite(blip, Config5599.Map[i].id)
        SetBlipDisplay(blip, 4)
        SetBlipColour(blip, Config5599.Map[i].color)
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, 1.0)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config5599.Map[i].name)
...
--	local manuel = _menuPool:AddSubMenu(menu, "Mode manuel","",true,true)
--Citizen.Wait(400)
...
function tenuxes(menu)
	...
	menu.OnItemSelect = function(_,ix,ind)
		...
		if ind == 1 then
			k = gettxt2("Tenue")
			if tostring(k) ~= nil then

				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
					TriggerServerEvent("dqp:SaveTenueS",k,skin)
				end)
				Wait(550)
				refreshthisshit()
			end
	...
	ESX.TriggerServerCallback('dqp:GetTenues', function(skin)
		if #skin == 0 then
			...
			m.OnItemSelect = function(_,ix,v)
				...
				if v == 2 then
					...
					if tostring(kx) ~= nil then
						TriggerServerEvent('dqp:RenameTenue', skin[i].id,kx)
						Wait(550)
						refreshthisshit()
					end
				end
				if v == 3 then
					TriggerServerEvent('dqp:DeleteTenue', skin[i].id)
					Wait(550)
					refreshthisshit()
				end
			end
...
function montreMenu(menu)
	...
	menu.OnIndexChange = function(menu,index6)
		...
		menu.OnListSelect = function(_, _, _)
			ESX.TriggerServerCallback("dqp:GetMoneyVet", function(result)
				if result then
	 ...
	 if curSex == 0 then
		...
		for i = 0,#chaussureItem,1 do
			...
			menu:AddItem(x)
				...
				-- 		SetPedComponentVariation(playerPed, 6, index6-1, 0, 2)
				-- 		local index2 = 1
				-- 		menu.OnListSelect = function(menu,item,index)
				-- 			print(index)
				-- 			ESX.TriggerServerCallback("dqp:GetMoneyVet", function(result)
				-- 				if result then
				-- 					TriggerEvent('skinchanger:getSkin', function(skin)
				...
				-- 		menu.OnListChange = function(menu,list,index23)
				-- 		index2 = index23
				-- 			SetPedComponentVariation(playerPed, 6, index6-1, index23-1, 2)
...
function boucleFct(menu)
	...
	menu.OnIndexChange = function(menu,index6)
		...
		index2 = 1
		menu.OnListSelect = function(_, _, _)
			TriggerServerEvent("dqp:SetNewMasque",index6-1,index2-1,"Boucle",boucleItem[index6],2)
			print(index2)
			-- TriggerEvent('skinchanger:getSkin', function(skin)
			...
			-- 	 clothesSkin = {
			-- 			['ears_1'] = index6-1, ['ears_2'] = index-1,


			-- 		}

			-- 		TriggerEvent('skinchanger:loadClothes', skin, clothesSkin)
			-- 		TriggerServerEvent("dqp:setEars",clothesSkin)
			-- 		TriggerEvent('skinchanger:getSkin', function(skin)
...
--	lunetteItem= {}
	...
	menu.OnIndexChange = function(menu,index6)
		...
		menu.OnListSelect = function(_, _, _)
			print(index2)
			TriggerServerEvent("dqp:SetNewMasque",index6-1,index2-1,"Lunette",lunetteItem[index6],1)
...
playerPed = GetPlayerPed(-1)
	...
	menu.OnIndexChange = function(menu,index6)
		...
		menu.OnListSelect = function(_, _, _)
			...
			print(pdka)
			print(pdka)
			TriggerServerEvent("dqp:SetNewMasque",index6-1,pdka,	"Chapeau",chapeauItem[index6],0)
