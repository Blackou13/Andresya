      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Cet article est déjà en vente dans votre magasin, ~r~mais pour ' .. data[1].price .. '. Vous mettez le prix ' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Retirer l article et mettre un nouveau prix')
            end
        end)  
    end)
end)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Vous avez commandé' .. count .. '  ' .. label .. ' pour $' .. price * count)
        else
            TriggerClientEvent('esx:showNotification', _source, '~r~Vous n avez pas assez d argent dans votre entreprise.')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g Vous avez acheté le magasin pour $' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~Vous ne pouvez acheter ce magasin')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~L argent de votre boutique a volé suite à un cambriolage, veuillez patienter ' .. math.floor((900 - time) / 60) .. ' minutes')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vous avez déposé $' .. amount .. ' dans votre magasin')
        else
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vous avez retiré $' .. amount .. ' de votre entreprise')
            xPlayer.addMoney(amount)
        else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Vous ne pouvez pas en retirer plus que ce que vous possédez')
        end
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vous avez vendu votre magasin')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Vous ne pouvez pas vendre votre magasin avec de l argent ou des objets dans votre stock')
            end
        end)
    end)
end)
