...
function OpenShopCenter()
	...
	ESX.UI.Menu.CloseAll()
  	...
  	local elements = {}
		...
		if showblip then
			table.insert(elements, {label = 'Masquer tous les magasins sur la carte', value = 'removeblip'})
		else
			table.insert(elements, {label = 'Afficher tous les magasins sur la carte', value = 'showblip'})
		end
...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Vous avez : $' .. data[1].money .. '',    value = ''})
		table.insert(elements, {label = 'Stock',    value = 'shipments'})
        table.insert(elements, {label = 'Mettre un objet à vendre', value = 'putitem'})
        table.insert(elements, {label = 'Retirer un objet de la vente',    value = 'takeitem'})
        table.insert(elements, {label = 'Déposer de l argent',    value = 'putmoney'})
        table.insert(elements, {label = 'Retirer de l argent',    value = 'takemoney'})
        table.insert(elements, {label = 'Changer le nom : $' .. Config.ChangeNamePrice,    value = 'changename'})
		table.insert(elements, {label = 'Vendre votre magasin pour $' .. math.floor(data[1].ShopValue / Config.SellValue),   value = 'sell'})
        ...
        elseif data.current.value == 'takemoney' then
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'takeoutmoney', {
                title = 'Combien voulez-vous en retirer ? '
            }, function(data2, menu2)
		...
		menu2.close()
	 	...
	 	elseif data.current.value == 'putmoney' then
			...
			title = 'Combien voulez-vous en déposer ? '
		...
		elseif data.current.value == 'sell' then
		  ...
		  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
			title = 'Ecrire YES pour confirmer la vente du magasin'
	  ...
	  elseif data.current.value == 'changename' then
		...
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'changename', {
		  title = 'Comment voulez-vous nommer votre magasin ? '
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Commander un produit', value = 'buy'})
	table.insert(elements, {label = 'Le stock', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '--STOCK PRESENT--'})
			table.insert(elements, {label = 'Obtenez tout votre stock', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '--STOCK EN ATTENTE--'})
				once = false
			end

			if time - items[i].time <= Config.DeliveryTime then
				times = time - items[i].time
				table.insert(elements, {label = items[i].label .. ' temps restant : ' .. math.floor((Config.DeliveryTime - times) / 60) .. ' minutes' })
			end
...
function OpenShipmentDelivery(id)
	...
	local elements = {}
		...
		for i=1, #Config.Items, 1 do
			table.insert(elements, {labels =  Config.Items[i].label, label =  Config.Items[i].label .. ' pour $' .. Config.Items[i].price .. '/unité ',	value = Config.Items[i].item, price = Config.Items[i].price})
		end
...
function TakeItem(number)
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getShopItems', function(result)
	...
	for i=1, #result, 1 do
	    if result[i].count > 0 then
	    	table.insert(elements, {label = result[i].label .. ' | ' .. result[i].count ..' objets en stock [' .. result[i].price .. ' $ par objet', value = 'removeitem', ItemName = result[i].item})
	    end
    end
...
local name = data.current.ItemName
    ...
    if data.current.value == 'removeitem' then
        ...
        title = 'Combien aimeriez-vous en retirer?'
        }, function(data2, menu2)
    ...
    for i=1, #result.items, 1 do
      ...
      local invitem = result.items[i]
	    ...
	    if invitem.count > 0 then
			table.insert(elements, { label = invitem.label .. ' | ' .. invitem.count .. ' dans votre inventaire', count = invitem.count, name = invitem.name})
	    end
	end
  ...
  function(data, menu)
        ...
        local invcount = data.current.count
			...
			if count > invcount then
				ESX.ShowNotification('~r~Vous ne pouvez pas vendre plus que ce que vous possédez')
				menu2.close()
				menu.close()
			else
				menu2.close()
...
Citizen.CreateThread(function()
	Citizen.Wait(500)
		...
		AddTextComponentString('Achat Superettes')
...
function createForSaleBlips()
	if showblip then
		...
		IDBLIPS = {
			...
			[2] = {x = 2557.458,  y = 382.282,  z = 108.622, n = 2},
			[3] = {x = -3038.939, y = 585.954,  z = 7.908, n = 3},
			[4] = {x = -1487.553, y = -379.107,  z = 39.163, n = 4},
			[5] = {x = 1392.562,  y = 3604.684,  z = 34.980, n = 5},
			[6] = {x = -2968.243, y = 390.910,   z = 14.043, n = 6},
			[7] = {x = 2678.916,  y = 3280.671, z = 55.241, n = 7},
			[8] = {x = -48.519,   y = -1757.514, z = 29.421, n = 8},
			[9] = {x = 1163.373,  y = -323.801,  z = 69.205, n = 9},
			[10] = {x = -707.501,  y = -914.260,  z = 19.215, n = 10},
			[11] = {x = -1820.523, y = 792.518,   z = 138.118, n = 11},
			[12] = {x = 1698.388,  y = 4924.404,  z = 42.063, n = 12},
			[13] = {x = 1961.464,  y = 3740.672, z = 32.343, n = 13},
			[14] = {x = 1135.808,  y = -982.281,  z = 45.415, n = 14},
			[15] = {x = 25.88,     y = -1347.1,   z = 29.5, n = 15},
...
function Robbery(id)
	...
	local coords1 = {
	[1] = {x = 380.74, y = 331.94, z = 103.566, heading = 255.47},
	[2] = {x = 2550.23, y = 387.34, z = 108.622, heading = 359.25},
	[3] = {x = -3047.88, y = 588.16, z = 7.908, heading = 20.34},
	[4] = {x = -1478.97, y = -374.36, z = 39.163, heading = 228.54},
	[5] = {x = 1395.49, y = 3612.94, z = 34.980, heading = 22.18},
	[6] = {x = -2959.5, y = 387.78, z = 14.043 , heading = 171.58},
	[7] = {x = 2674.36, y = 3288.0, z = 55.241, heading = 339.44},   
	[8] = {x = -42.64,   y = -1749.34, z = 29.41, heading = 337.22},	
	[9] = {x = 1160.77, y = -314.03, z = 69.205, heading = 10.81},  
	[10] = {x = -708.14, y = -904.05, z = 19.215 , heading = 5.29}, 
	[11] = {x = -1828.23, y = 799.83, z = 138.15, heading = 53.63}, 
	[12] = {x = 1706.87, y = 4919.76, z = 42.063, heading = 237.41}, 
	[13] = {x = 1960.75, y = 3748.67, z = 32.34, heading = 304.34}, 
	[14] = {x = 1126.26, y = -980.84, z = 45.42, heading = 9.13}, 
	[15] = {x = 30.45, y = -1339.88, z = 29.49 , heading = 269.79}, 
	...
	[20] = {x = 1736.66, y = 6419.02, z = 35.03, heading = 247.6},  
	}
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
                    ...
                    local coords = {
						...
						TriggerServerEvent('esx_phone:send', "police", "Vol de magasin au " .. result.name .. '\'magasin', true, coords)
						TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~r~Votre magasin ~b~(" .. result.name .. ')~r~ est entrain de se faire cambrioler', id)
					...
					ESX.ShowNotification("~r~Il n y a pas assez de policiers en ville " .. results .. '/' .. Config.RequiredPolices)
				end
			else
				ESX.ShowNotification("~r~Ce magasin a déjà été volé, veuillez patienter " ..  math.floor((Config.TimeBetweenRobberies - result.time)  / 60) .. ' minutes')
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					TriggerEvent("mt:missiontext", "Voler la recette en cassant le coffre :~r~ " .. hp/200 .. "%", 1000)

				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('esx_kr_shops-robbery:GetReward', Id)
					TriggerServerEvent("esx_kr_shops-robbery:NotifyOwner", '~r~Le vol sur votre boutique ~b~(' .. Name ..')~r~ a malheureusement été réussi !', Id)
					DeleteEntity(Var)
				end
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~g~Le vol sur votre boutique ~b~(" .. Name .. ')~g~ n a pas été réussi !', Id)
