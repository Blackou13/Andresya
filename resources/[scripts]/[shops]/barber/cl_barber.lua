...
function OpenShopMenuS(menu)
	local chev = _menuPool:AddSubMenu(menu, "Cheveux","",true,true,true)
	local barb = _menuPool:AddSubMenu(menu, "Barbes","",true,true,true)
	local maq = _menuPool:AddSubMenu(menu, "Maquillage","",true,true,true)

	MegaPute(barb)
	GigaPutePute(maq)
	asdhbjfinfdsaxbjhkiasbjnd5985(chev)
end
function GigaPutePute(menu)
	ppp = {
	...
	local makeupHighColor = NativeUI.CreateColourPanel("Deuxième couleur du maquillage", Colors)
		...
		menu.OnItemSelect = function(_, _, _)
			...
			ESX.TriggerServerCallback("dqp:GetCoiffeurMontant", function(result)
				if result then
					TriggerEvent('skinchanger:getSkin', function(skin)
						...
						ESX.ShowNotification("~g~Nouveau maquillage appliqué")
					end)
...
function MegaPute(menu)
	...
	local beardSelect = NativeUI.CreateListItem("Barbe", p, 0)
		...
		ESX.TriggerServerCallback("dqp:GetCoiffeurMontant", function(result)
			if result then
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					skin.beard_4 = pp[4]
					TriggerServerEvent('esx_skin:save', skin)
					ESX.ShowNotification("~g~Nouvelle barbe appliquée")
				end)
...
function asdhbjfinfdsaxbjhkiasbjnd5985(menu)
	...
	local hairSelect = NativeUI.CreateListItem("Cheveux", hairstyles, 0)
	local hairBaseColor = NativeUI.CreateColourPanel("Couleur de base", Colors)
		...
		ESX.TriggerServerCallback("dqp:GetCoiffeurMontant", function(result)
			if result then
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					TriggerServerEvent('esx_skin:save', skin)
					ESX.ShowNotification("~g~Nouvelle coiffure appliquée")
				end)
...
AddEventHandler('esx_barbershop:hasEnteredMarker', function(_)
	...
	CurrentActionMsg  = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au coiffeur'
	CurrentActionData = {}
end)
