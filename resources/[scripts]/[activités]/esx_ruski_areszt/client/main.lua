...
ESX               				= nil
local PlayerData                = {}
local PracaPolice 				= 'police'	-- Travail pouvant supporter le système d'animation

local Aresztuje					= false		-- Quitter False sinon il sera arrêté au début du script
local Aresztowany				= false		-- Laissez False sinon vous serez arrêté au début du script
	...
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['E']) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and PlayerData.job.name == PracaPolice then	-- Mozesz tutaj zmienic przyciski
			...
			if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				ESX.ShowNotification("~b~Vous arrêtez un citoyen~r~ " .. GetPlayerServerId(closestPlayer) .. "")						
				...
				Citizen.Wait(3100)																									
				ESX.ShowNotification("~b~Citoyen arrêté ~r~ " .. GetPlayerServerId(closestPlayer) .. "")					
