...
AddEventHandler('esx_loffe_fangelse:hasEnteredMarker', function (zone)
  if zone == 'StartJob' then
    ...
    CurrentActionMsg  = ('Appuyez sur ~INPUT_CONTEXT~ pour commencer à travailler.')
    CurrentActionData = {}
  elseif zone == 'LeaveBox' then
    CurrentAction	  = 'leave_box'
	CurrentActionMsg  = ('Appuyez sur ~INPUT_CONTEXT~ pour livrer le carton..')
	CurrentActionData = {}
  elseif zone == 'TeleportIn' then
    CurrentAction	  = 'teleport_in'
	CurrentActionMsg  = ('~INPUTCONTEXT~')
	CurrentActionData = {}
  elseif zone == 'TeleportUt' then
    CurrentAction	  = 'teleport_ut'
	CurrentActionMsg  = ('~INPUT_CONTEXT~')
	CurrentActionData = {}
  end
end)
    ...
    if IsControlPressed(0,  Keys['E']) then
        ...
        if CurrentAction == 'start_job' then
		 ...
		 if CurrentAction == 'teleport_in' then
			Citizen.Wait(1000)
           SetEntityCoords(GetPlayerPed(-1), 1800.69, 2483.15, -123.54)
...
AddEventHandler('esx_loffe_fangelse:startJob', function()
	...
	repeat
	--TriggerEvent("mt:missiontext", '~w~Emballe la boîte. Temps restant: ~b~' .. TimeLeft .. ' ~w~', 1000)
	TimeLeft = TimeLeft - 1
	Citizen.Wait(1000)
	until(TimeLeft == 0)
