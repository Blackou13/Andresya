...
AddEventHandler('sendProximityMessageDo', function(id, name, message)
  ...
  local pid = GetPlayerFromServerId(id)
  if pid == myId then
    TriggerEvent('chatMessage', "🚑 INFO EMS", {255, 0, 0}, " ^*^0 " .. name .. "^r^0".. " " .. message.. " ")
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 19.999 then
    TriggerEvent('chatMessage', "🚑 INFO EMS", {255, 0, 0}, " ^*^0 " .. name .. "^r^0".. " " .. message.. " ")
  end
end)
...
AddEventHandler('sendProximityMessageMedd', function(id, name, message)
  ...
  if pid == myId then
    TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(76, 35, 86, 0.9); border-radius: 3px; margin-left: 0; margin-right: 0;">^*{0}  * ^r{1} * </div>',
      args = { ''..id..' | ', message }
  })
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 19.999 then
    TriggerEvent('chat:addMessage', {
...
AddEventHandler('sendProximityMessageMeddd', function(id, name, message)
  ...
  if pid == myId then
    TriggerEvent('chat:addMessage', {
      template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(36, 36, 36, 0.9); border-radius: 3px; margin-left: 0; margin-right: 0;">^*{0}  * ^r{1} * </div>',
      args = { ''..id..' | ', message }
  })
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 19.999 then
    TriggerEvent('chat:addMessage', {
