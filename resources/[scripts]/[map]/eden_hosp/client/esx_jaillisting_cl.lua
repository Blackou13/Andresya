...
function ShowJailListingMenu(data)
		...
		ESX.UI.Menu.Open(
			...
			{
				css = 'vetement',
				title    = _U('hopital_center'),
				elements = {
				{label = _U('citizen_wear'), value = 'citizen_wear'},
				{label = _U('hopital_wear'), value = 'hopital_wear'},
			},
			},
			function(data, menu)
	...
	while true do
		...
		for i=1, #Config.Zones, 1 do
			if(GetDistanceBetweenCoords(coords, Config.Zones[i].x, Config.Zones[i].y, Config.Zones[i].z, true) < Config.ZoneSize.x) then
            	...
            	AddTextComponentString(_U('access_hopital_center'))
