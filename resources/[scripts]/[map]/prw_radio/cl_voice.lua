local VoiceMode = {
	{ dist = 3, message = "Distance 3 mètres." },
	{ dist = 8, message = "Distance 8 mètres." },
	{ dist = 14, message = "Distance 14 mètres." },
	{ veh = true, dist = 4, func = function(ped) return IsPedInAnyVehicle(ped) end, message = "Portée vocale réglée sur votre véhicule" },
}
local Keys = {
...
function OpenRadioMenu()
    ...
    if radioMenu == nil then
        radioMenu = NativeUI.CreateMenu("Radio", "Frequence", 5, 200)
...
function AddRadioMenu(menu)
    ...
    pp = NativeUI.CreateItem('Frequence', "")
    pp:RightLabel(freq)
    menu:AddItem(pp)
    menu.OnItemSelect = function(m, i, ind)
        if ind == 1 then
            --drawnotifcolor("Plage de fréquence incorrecte (plage 0 à 100)", 26)
            dm = gettxt2(freq)
            drawnotifcolor("Fréquence préréglée : " .. tostring(dm), 18)
            dm = tonumber(dm)
            if dm ~= nil then
                if dm > 0 then
                    dm = round2(dm, 2)
                    if dm <= 100 then
                        ...
                        drawnotifcolor("Plage de fréquence incorrecte (plage 0 à 100)", 6)
                    end
                end
            end
        end
        ...
        if ind == 2 then
            drawnotifcolor("Fréquence réglée sur " .. freq, 18)
			...
			ppks = NativeUI.CreateCheckboxItem('Statut Radio', muet, "")
            menu:AddItem(ppks)
			_menuPool:RefreshIndex()
			end
        end
