...
function OpenPersonnelMenu()
	...
	ESX.TriggerServerCallback('NB:getUsergroup', function(group)
		...
		table.insert(elements, {label = 'Cacher/afficher HUD',		value = 'menuperso_hud'})
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if data.current.value == 'menuperso_moi' then
					...
					table.insert(elements, {label = 'Mes licences',							value = 'menuperso_moi_licences'})
					...
					table.insert(elements, {label = 'Mes papiers',                            value = 'idcard'})
					...
					ESX.UI.Menu.Open(
						...
						function(data2, menu2)
							if data2.current.value == 'menuperso_moi_licences' then
								ESX.UI.Menu.Open(
									'default', GetCurrentResourceName(), 'menuperso_gpsrapide',
									{
										...
										align    = 'top-left',
										elements = {
											{label = 'Montrer Licence 1 ',     value = 'licence_1'},
											{label = 'Montrer Licence 2 ',     value = 'licence_2'},
											{label = 'Montrer Licence 3 ',     value = 'licence_3'},
										},
									},
									function(data3, menu3)
			
										if data3.current.value == 'licence_1' then
											local player, distance = ESX.Game.GetClosestPlayer()

											if distance ~= -1 and distance <= 3.0 then
												TriggerServerEvent('esx_armurierjob:showLicence1', GetPlayerServerId(player))
											end
										end
			
										if data3.current.value == 'licence_2' then
											local player, distance = ESX.Game.GetClosestPlayer()

											if distance ~= -1 and distance <= 3.0 then
												TriggerServerEvent('esx_armurierjob:showLicence2', GetPlayerServerId(player))
											end
										end
			
										if data3.current.value == 'licence_3' then
											local player, distance = ESX.Game.GetClosestPlayer()

											if distance ~= -1 and distance <= 3.0 then
												TriggerServerEvent('esx_armurierjob:showLicence3', GetPlayerServerId(player))
											end
										end
				...
				if data.current.value == 'menuperso_hud' then
					hudstat = not hudstat
					TriggerEvent("showhud:toggle",hudstat)
				end
...
function OpenMenuMask()
	...
	ESX.TriggerServerCallback("dqp:getMask", function(result2)
		...
		local SkurtShop = NativeUI.CreateMenu("Mes Accessoires", "accessoires disponibles", 5, 100)
